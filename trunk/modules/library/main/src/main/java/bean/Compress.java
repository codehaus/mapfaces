		...
		Process tmp ;
			...
			for(int i=0 ; i<files.length; i++){
				if(files[i].getName().endsWith(".js") && !files[i].isDirectory()){
					...
					for(int j=0 ; j<noFiles.length; j++){
						if((files[i].getPath().contains(noFiles[j]))
								|| files[i].getPath().contains("GmlRendererWZ.js")
								|| files[i].getPath().contains("MapbuilderCompressed.js")
								|| files[i].getPath().contains("MapbuilderServerLoad.js")){ breaked = true;break;}
					}	
					if(!breaked){
							try {
			...
			for(int i=0 ; i<files.length; i++){
				if(files[i].isDirectory() 
						&& !files[i].getName().contains("overlib")
						&& !files[i].getName().contains("graphics")
						&& !files[i].getName().contains("xsl")
						&& !files[i].getName().contains("wz_jsgraphics")
						&& !files[i].getName().contains("overlib")
						&& !files[i].getName().contains("schemas")
						&& !files[i].getName().contains("skin")
						&& !files[i].getName().contains("sarissa")){
