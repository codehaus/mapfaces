...
import org.mapfaces.util.portrayThread;
...
public class LayerRenderer extends WidgetBaseRenderer {
    ...
    public void encodeBegin(FacesContext context, UIComponent component) throws IOException {
        ...
        try {
            ...
            writer.writeAttribute("style", display + "position: absolute; width: 100%; height: 100%; z-index: 100;" + styleImg + comp.getStyle(), "style");
                ...
                //Add layer image if we want the HtmlGraphicImage be in the JSF tree     
                writer.startElement("div", comp);
                ...
                if(comp.getChildCount() > 0  && comp.getChildren().get(0) instanceof HtmlGraphicImage){
                    ((HtmlGraphicImage) comp.getChildren().get(0)).setUrl("/resource.jsf?r=/org/mapfaces/resources/img/Spacer.gif");
                    ThreadPhaseListener.setImgUrl(((HtmlGraphicImage) comp.getChildren().get(0)).getId(), "noImage");
                }else{   
                    HtmlGraphicImage img = new HtmlGraphicImage();
                    img.setId(id + "_Img");
                    img.setStyleClass("layerImg");
                    img.setUrl("/resource.jsf?r=/org/mapfaces/resources/img/Spacer.gif");
                    comp.getChildren().add(img);
                    ThreadPhaseListener.setImgUrl(img.getId(), "noImage");
                }
            ...
            if (FacesUtils.getParentUIMapPane(context, component).getInitDisplay() && !layer.isHidden()) {
                ...
                if (defaultMapContext.layers().add(mapLayer)) {      
                    ...
                    portrayThread tempThread = new portrayThread(context, component, FacesUtils.getParentUIMapPane(context, component).getPortray(),defaultMapContext, env, dst, layer.getOutputFormat(), new Dimension(new Integer(width), new Integer(height)), false);
                    tempThread.start();
                }                
            }
            writer.flush();
    ...
    public void encodeEnd(FacesContext context, UIComponent component) throws IOException {
        ...
        ResponseWriter writerr = context.getResponseWriter();
        
        writerr.endElement("div");
        writerr.endElement("div");
        writerr.flush();
    }
