	...
	*/
		...
		var t=bbox.ur.lat;		
		 ...
		 //test if coords are valid
		  ...
		  while(lo<=r){
				...
				{	d.title = " W";//mbGetMessage("westWgs84");
					d.innerHTML = (Math.abs(lo)).toFixed(lonDecs)+" W";
				 }
				 else 
				{	d.title = "E"; //mbGetMessage("eastWgs84");
	...
	var j = 0;
	  ...
	  while(b<=t){
			...
			{	d.title = " S";//mbGetMessage("southWgs84");
				d.innerHTML = (Math.abs(b)).toFixed(latDecs)+" S";
			}
			 else 
			{	d.title = "N ";//mbGetMessage("northWgs84");
	...
	*/  
		...
		*/ 
		update : function (){
			...
			this.bbox=this.map.getExtent().toArray();
