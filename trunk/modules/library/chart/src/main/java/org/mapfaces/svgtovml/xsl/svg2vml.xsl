...
<!DOCTYPE xsl:stylesheet [
    <!ENTITY path  "resource.jsf?r=/org/mapfaces/svgtovml/xsl/" ><!-- small n, tilde -->
]>
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:svg="http://www.w3.org/2000/svg" xmlns:v="urn:schemas-microsoft-com:vml" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns="http://www.w3.org/1999/xhtml" xmlns:xlink="http://www.w3.org/1999/xlink">
	<!-- path:resource.jsf?r=/org/mapfaces/svgtovml/xsl/
    INDEX:
   * template:
        - svg-g 
        - svg-use
...
-->
    <xsl:variable name="schermo-x">
        <xsl:value-of select="//svg:g[@id='canvas']/@width"/>
    </xsl:variable>
    <xsl:variable name="schermo-y">
        <xsl:value-of select="//svg:g[@id='canvas']/@height"/>
    </xsl:variable>
        ...
        <html>
            ...
            <body onload="if(window.init)window.init();" >
                <xsl:attribute name="style">
                    ...
                    <xsl:choose><!-- se ho degli elementi svg ancestor, e ho width, devo convertirlo in user unit -->
                        <xsl:when test="$schermo-x">
    ...
    <xsl:template match="svg:svg">
        <xsl:choose>
            <xsl:when test="((count(ancestor::svg:svg) = 0) ) or 
...
-->
                <xsl:element name="v:group"  >
                    ...
                    <xsl:attribute name="style">
                        ...
                        <xsl:choose><!-- se ho degli elementi svg ancestor, e ho width, devo convertirlo in user unit -->
                            ...
                            </xsl:when><!-- in questo caso, sono nel primo elemento svg, per cui width non dev'essere convertito in 
         user unit -->
                            <xsl:when test="@width">
                                <xsl:value-of select="@width"/>
                            </xsl:when><!-- se non ho width lo determino in base agli elementi svg precedenti -->
   ...
   -->
	...
	<!-- ********************************************************************************* -->
	<!-- ********************************************************************************* -->
    <xsl:template match="svg:style" mode="head">
    ...
    <xsl:template match="svg:script" mode="head">
        <script>
            <xsl:attribute name="type">
                <xsl:value-of select="@type"/>
            </xsl:attribute>
            ...
            <![CDATA[/*IE need to have something on its SCRIPT tag*/]]>
        </script>
    </xsl:template>
	<!-- DA TOGLIERE -->
    <xsl:template match="*" mode="head">
		<!-- cerca tutti gli elementi, se è presente style crea l'elemento style -->
        <xsl:apply-templates select="svg:style" mode="head"/>
        <xsl:apply-templates select="svg:script" mode="head"/>
        <!--xsl:for-each select="svg:style">
            <style>
                ...
                <!-- cerca tutti gli elementi, se è presente style crea l'elemento script -->
...
xlink:show, xlink:actuate -->
    ...
    <xsl:template name="coord-origin-size">
        ...
        (substring-after(normalize-space(@viewBox),' '),' '),' ')"/>
				...
				<!-- *********************************************************************** -->
                <xsl:attribute name="coordorigin"><!-- primi due valori di viewBox -->
                    <xsl:value-of select="concat($val-left,' ',$val-top)"/>
                </xsl:attribute>
                <xsl:attribute name="coordsize"><!-- ultimi due valori di viewBox -->
    ...
    -->
            <xsl:otherwise>
                ...
                <xsl:attribute name="coordsize"><!-- primo valore di coordsize --><!-- valore di width -->
                    <xsl:value-of select="$w-temp"/>
                    <xsl:text> </xsl:text><!-- secondo valore di coordsize --><!-- valore di height -->
...
-->
	...
	<!-- ****************************************************************************** -->
	<!-- ****************************************************************************** -->
	<!-- MAPFACES template to load chart attributes -->
    <xsl:template name="svg-chartAttributes">
        <xsl:if test="@serie">
	...
	<!-- ****************************************************************************** -->
	<!-- ****************************************************************************** -->
    <xsl:include href="&path;XSL2/rect.xsl"/>
    <xsl:include href="&path;XSL2/dimensioni.xsl"/>
    <xsl:include href="&path;XSL2/transform.xsl"/>
    <xsl:include href="&path;XSL2/xywh.xsl"/>
    <xsl:include href="&path;XSL2/predef.xsl"/>
    <xsl:include href="&path;XSL2/attributi-paint.xsl"/>
    <xsl:include href="&path;XSL2/text.xsl"/>
    <xsl:include href="&path;XSL2/text-dim.xsl"/>
    <xsl:include href="&path;XSL2/text-elemento.xsl"/>
    <xsl:include href="&path;XSL2/text-ric.xsl"/>
    <xsl:include href="&path;XSL2/gradient-pattern.xsl"/>
    <xsl:include href="&path;XSL2/marker.xsl"/>
    <xsl:include href="&path;XSL2/filter.xsl"/>
    <xsl:include href="&path;XSL2/gestione-style.xsl"/>
    <xsl:include href="&path;XSL2/path.xsl"/>
    <xsl:include href="&path;XSL2/matematica.xsl"/>
        <!-- ********************************************************************************* -->
</xsl:stylesheet>
