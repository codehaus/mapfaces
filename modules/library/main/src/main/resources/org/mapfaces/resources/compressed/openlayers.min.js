...
OpenLayers.i18n=OpenLayers.Lang.translate;OpenLayers.Lang.setCode=function(code){var lang;
if(!code){if(window.language){code=language
}else{code=(OpenLayers.Util.getBrowserName()=="msie")?navigator.userLanguage:navigator.language
}}var parts=code.split("-");
parts[0]=parts[0].toLowerCase();
if(typeof OpenLayers.Lang[parts[0]]=="object"){lang=parts[0]
}if(parts[1]){var testLang=parts[0]+"-"+parts[1].toUpperCase();
...
OpenLayers.Map.TILE_HEIGHT=256;OpenLayers.Util.trim=function(str){var str=str.replace(/^\s\s*/,""),ws=/\s/,i=str.length;
while(ws.test(str.charAt(--i))){}return str.slice(0,i+1)
};OpenLayers.Handler.MouseWheel=OpenLayers.Class(OpenLayers.Handler,{wheelListener:null,mousePosition:null,initialize:function(control,callbacks,options){OpenLayers.Handler.prototype.initialize.apply(this,arguments);
this.wheelListener=OpenLayers.Function.bindAsEventListener(this.onWheelEvent,this)
},destroy:function(){OpenLayers.Handler.prototype.destroy.apply(this,arguments);
this.wheelListener=null
},onWheelEvent:function(e){if(!this.map||!this.checkModifiers(e)){return 
...
return this.div
},redraw:function(evt){this.div.style.zIndex=this.map.Z_INDEX_BASE.Control;
var lonLat;
var newHtml="";
if(evt==null){lonLat=new OpenLayers.LonLat(0,0);
...
var newHtml=this.prefix+"<span class='mfCursorTrackPrefix'>"+this.prefixLat+"</span>"+lonLat.lat.toFixed(digits)+this.separator+"<span class='mfCursorTrackPrefix'>"+this.prefixLon+"</span>"+lonLat.lon.toFixed(digits)+this.suffix;
return newHtml
...
var newHtml=this.prefix+"<span class='mfCursorTrackPrefix'>"+this.prefixLat+"</span>"+lat[0]+this.separatorDM+lat[1]+this.separatorMS+lat[2]+this.separatorSHemi+lat[3]+this.separator+"<span class='mfCursorTrackPrefix'>"+this.prefixLon+"</span>"+lon[0]+this.separatorDM+lon[1]+this.separatorMS+lon[2]+this.separatorSHemi+lon[3]+this.suffix;
return newHtml
...
var newHtml=this.prefix+"<span class='mfCursorTrackPrefix'>"+this.prefixLat+"</span>"+lat[0]+this.separatorDM+lat[1]+this.separatorMS+lat[2]+this.separator+"<span class='mfCursorTrackPrefix'>"+this.prefixLon+"</span>"+lon[0]+this.separatorDM+lon[1]+this.separatorMS+lon[2]+this.suffix;
return newHtml
...
},update:function(scale){if(this.map==null||(this.map.baseLayer==null)||!this.map.getScale()){return 
...
}if(options.zoomOut){tab.push(new OpenLayers.Control.ZoomBox({out:true,displayClass:"olControlZoomBoxOut"}))
}if(options.pan){(options.panEffect)?tab.push(new OpenLayers.Control.Navigation({animatedPanEnabled:true})):tab.push(new OpenLayers.Control.Navigation())
}if(options.zoomMaxExtent){tab.push(new OpenLayers.Control.ZoomToMaxExtent())
}if(options.history&&nav!=null){tab.push(nav.previous);
tab.push(nav.next)
...
if(this.controls&&(this.controls.length>0)){for(var i=0;
i<this.controls.length;
i++){if(this.controls[i].panel_div&&!this.controls[i].panel_div.title){var tooltipClass=this.controls[i].displayClass;
if(tooltipClass.indexOf(" ")!=-1){var tmp=this.controls[i].displayClass.split(" ");
...
OpenLayers.Lang.en.olControlGetFeatureInfoToolTip="Query a layer";OpenLayers.Lang.fr.srs="Srs: ";
...
OpenLayers.Lang.fr.olControlGetFeatureInfoToolTip="Interroger une couche";function loadMap(id){if(id.indexOf(":")!=-1){id=id.replace(":","")
}eval("if(window.loadMap"+id+")window.loadMap"+id+"();")
}function loadMapAndZoom(id){loadMap(id);
window.maps[id].zoomToExtent(window.maps[id].currentExtent)
}function addControlsToMap(id){var controls=eval("if(window.controlToAdd"+id+")window.controlToAdd"+id);
if(controls){for(var i=0;
i<controls.length;
i++){controls[i]()
}}}function reloadAllMaps(){if(window.maps){for(var i in window.maps){var map=window.maps[i];
if(map&&map.div&&map.div.style.display!="none"){if(map.currentExtent){map.zoomToExtent(map.currentExtent)
}else{map.zoomToMaxExtent()
}}else{loadMapAndZoom(i);
addControlsToMap(i)
}}}};