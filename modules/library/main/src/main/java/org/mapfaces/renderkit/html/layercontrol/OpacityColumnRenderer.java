...
public class OpacityColumnRenderer extends SelectOneMenuColumnRenderer {
    ...
    public void handleAjaxRequest(FacesContext context, UIComponent component) {
        UIOpacityColumn comp = (UIOpacityColumn) component;
        AjaxUtils ajaxtools = new AjaxUtils();
        ...
        for (Method method : theclass.getMethods()) {
            if (method.getName().equals("set" + propName)) {
                try {
                    ...
                    Logger.getLogger(OpacityColumnRenderer.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public String addBeforeRequestScript(String layerId) {
        return "document.getElementById('" + layerId + "').style.opacity=event.target.value;"
//               +"var ieValue=event.target.value*100;" +
