...
/**
 * @requires OpenCharts/Renderer.js
 */

/**
 * Class: OpenCharts.ElementsIndexer
 * This class takes care of figuring out which order elements should be
 *     placed in the DOM based on given indexing methods. 
 */
OpenCharts.ElementsIndexer = OpenCharts.Class({
    ...
    initialize: function(yOrdering) {
        ...
        this.compare = yOrdering ? 
            OpenCharts.ElementsIndexer.IndexingMethods.Z_ORDER_Y_ORDER :
            OpenCharts.ElementsIndexer.IndexingMethods.Z_ORDER_DRAWING_ORDER;
    ...
    insert: function(newNode) {
        ...
        while (rightIndex - leftIndex > 1) {
            ...
            var placement = this.compare(this, newNode,
                OpenCharts.Util.getElement(this.order[middle]));
        ...
        return nextIndex < this.order.length ?
                OpenCharts.Util.getElement(this.order[nextIndex]) : null;
    },
...
/**
 * Namespace: OpenCharts.ElementsIndexer.IndexingMethods
 * These are the compare methods for figuring out where a new node should be 
 *     placed within the indexer. These methods are very similar to general 
 *     sorting methods in that they return -1, 0, and 1 to specify the 
 *     direction in which new nodes fall in the ordering.
 */
OpenCharts.ElementsIndexer.IndexingMethods = {
     ...
     * 
     * Parameters:
     * indexer - {<OpenCharts.ElementsIndexer>}
     * newNode - {DOMElement}
     * nextNode - {DOMElement}
     * 
     * Returns:
    ...
    Z_ORDER_DRAWING_ORDER: function(indexer, newNode, nextNode) {
        var returnVal = OpenCharts.ElementsIndexer.IndexingMethods.Z_ORDER(
            indexer, 
            newNode, 
            nextNode
        );
 ...
 *  - <OpenCharts.Renderer>
 */
OpenCharts.Renderer.Elements = OpenCharts.Class(OpenCharts.Renderer, {
    ...
    /**
     * Property: Indexer
     * {<OpenCharts.ElementIndexer>} An instance of OpenCharts.ElementsIndexer 
    ...
    drawGeometry: function(geometry, style, featureId) {
        ...
        var rendered = true;
        if ((className == "OpenCharts.Geometry.Collection") ||
            (className == "OpenCharts.Geometry.MultiPoint") ||
            (className == "OpenCharts.Geometry.MultiLineString") ||
            (className == "OpenCharts.Geometry.MultiPolygon")) {
    ...
    redrawBackgroundNode: function(id, geometry, style, featureId) {
        var backgroundStyle = OpenCharts.Util.extend({}, style);
...
alert('icila');},
     ...
     * 
     * Parameters:
     * evt - {Object} An <OpenCharts.Event> object
     *
     * Returns:
     * {<OpenCharts.Geometry>} A geometry from an event that 
     *     happened on a layer.
     */
    ...
    eraseGeometry: function(geometry) {
        if ((geometry.CLASS_NAME == "OpenCharts.Geometry.MultiPoint") ||
            (geometry.CLASS_NAME == "OpenCharts.Geometry.MultiLineString") ||
            (geometry.CLASS_NAME == "OpenCharts.Geometry.MultiPolygon") ||
            (geometry.CLASS_NAME == "OpenCharts.Geometry.Collection")) {
            for (var i=0, len=geometry.components.length; i<len; i++) {
                this.eraseGeometry(geometry.components[i]);
            }
        } else {    
            var element = OpenCharts.Util.getElement(geometry.id);
    ...
    nodeFactory: function(id, type) {
        var node = OpenCharts.Util.getElement(id);
        if (node) {
...
/**
 * Constant: OpenCharts.Renderer.symbol
 * Coordinate arrays for well known (named) symbols.
 */
OpenCharts.Renderer.symbol = {
